...
int is_decimal_zero(s21_decimal *dcml) {
  if (!dcml->bits[0] && !dcml->bits[1] && !dcml->bits[2]) {
    set_sign(dcml, 0);  // ставим знак у нуля '+'
    return TRUE;
  }
...
int equal(s21_decimal *value_1, s21_decimal *value_2, int *mismatch) {
  ...
  *mismatch = FALSE;
  ...
  if (sign_1 == sign_2) {
    for (i = MANTISSA_BITS_SIZE - 1; i >= 0; --i) {
...
int initial_comparison_check(s21_decimal value_1, s21_decimal value_2) {
  ...
  } else if (!get_sign(value_1) && is_decimal_zero(&value_2)) {
    output = GREATER_SIGN;

  } else if (is_decimal_zero(&value_1) && get_sign(value_2)) {
    output = GREATER_SIGN;
...
int is_big_decimal_zero(s21_big_decimal *dcml) {
  if (!dcml->bits[0] && !dcml->bits[1] && !dcml->bits[2] && !dcml->bits[3] &&
    ...
    dcml->sign = 0;  // ставим знак у нуля '+'
    return TRUE;
  }
...
int equal_big_decimal(s21_big_decimal *value_1, s21_big_decimal *value_2,
  ...
  *mismatch = FALSE;
  ...
  if (value_1->sign == value_2->sign) {
    for (i = MANTISSA_BITS_SIZE - 1; i >= 0; --i) {
      if (get_bit_big_decimal(*value_1, i) !=
...
int initial_comparison_check_big_decimal(s21_big_decimal value_1,
  ...
  if (is_big_decimal_zero(&value_1) && is_big_decimal_zero(&value_2)) {
    output = VALUES_ZERO;

  } else if (value_1.sign == value_2.sign) {
    output = EQUAL_SIGN;

  } else if (!value_1.sign && is_big_decimal_zero(&value_2)) {
    output = GREATER_SIGN;

  } else if (is_big_decimal_zero(&value_1) && value_2.sign) {
    output = GREATER_SIGN;
...
int is_less_big_decimal(s21_big_decimal value_1, s21_big_decimal value_2) {
  int output = initial_comparison_check_big_decimal(value_1, value_2);
  if (output == GREATER_SIGN || output == VALUES_ZERO) {
    return FALSE;
  } else if (output == LESS_SIGN) {
    return TRUE;
  }

  int mismatch = FALSE;
  int i = equal_big_decimal(&value_1, &value_2, &mismatch);
  if (mismatch && !get_bit_big_decimal(value_1, i)) {
    output = TRUE;
  }
  if (value_1.sign && value_2.sign && mismatch) {
    output ^= 1;
  }
  return output;
}
...
int add_sub_big_decimal(s21_decimal value_1, s21_decimal value_2,
  ...
  is_decimal_zero(result);  // если нуль, ставим знак у нуля '+'
...
int div_mantissa_big_decimal(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  s21_big_decimal tmp_bit_value_1 = value_1;
  const int num_bits_mantissa = num_of_bits_in_the_decimal_value(value_1);
  const int empty_bits = MANTISSA_BIG_BITS_SIZE - num_bits_mantissa;
  ...
  for (int i = MANTISSA_BIG_BITS_SIZE - 1; i >= 0; --i) {
    ...
    } else if (i <= (empty_bits - 1) && !is_big_decimal_zero(&tmp)) {
      left_shift_big_decimal(&tmp);
      left_shift_big_decimal(result);
