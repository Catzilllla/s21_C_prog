...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  int sign_operations = 0;  // т.к. мы в функции - <<сложение>>
...
int s21_sub(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  int sign_operations = 1;  // т.к. мы в функции - <<разность>>
  int output = add_sub_big_decimal(value_1, value_2, &sign_operations, result);
...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  tmp_result.scale =
      big_value_1.scale + big_value_2.scale;  // scale с учетом умножения
